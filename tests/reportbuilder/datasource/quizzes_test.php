...
use local_activitysetting\reportbuilder\datasource\quizzes;
 ...
 * @covers \local_activitysetting\reportbuilder\datasource\quizzes
 */
final class quizzes_test extends core_reportbuilder_testcase {
    ...
    public function test_datasource_default(): void {
        ...
        $report = $generator->create_report(['name' => 'My report', 'source' => quizzes::class, 'default' => 1]);

        $content = $this->get_custom_report_content($report->get('id'));
        $this->assertCount(3, $content);

        // Default columns are course fullname, quizname, groupmode.
        // Sorted by name ascending.
        $this->assertEquals([
            [$course->fullname, $quiz1->name, get_string('groupsnone', 'core')],
            [$course->fullname, $quiz2->name, get_string('groupsnone', 'core')],
            [$course->fullname, $quiz3->name, get_string('groupsnone', 'core')],
    ...
    public function test_datasource_non_default_columns(): void {
        ...
        /** @var core_reportbuilder_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('core_reportbuilder');
        $report = $generator->create_report(['name' => 'My report', 'source' => quizzes::class, 'default' => 0]);

        $generator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'quiz:quizname']);
        $generator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'quiz:attemptsallowed']);
        ...
        // The columns are: quizname, maxattempts, completion, completionview.
        // Sorted by name ascending.
        $this->assertEquals([
            [$quiz1->name, 1, get_string('completion_automatic', 'completion'), get_string('yes')],
        ...
        // Sorted by name ascending.
        $this->assertEquals([
            [$quiz2->name, 2, get_string('completion_manual', 'completion'), get_string('no')],
        ...
        // Sorted by name ascending.
        $this->assertEquals([
            [$quiz3->name, 3, get_string('completion_none', 'completion'), get_string('no')],
    ...
    public function test_stress_datasource(): void {
        ...
        $this->datasource_stress_test_columns(quizzes::class);
        $this->datasource_stress_test_columns_aggregation(quizzes::class);
        $this->datasource_stress_test_conditions(quizzes::class, 'quiz:quizname');
    }
}
